...
//            NSString *dataDay  = NSEaseLocalizedString(@"ui.today", @"Today");
//            NSLog(@"%@",dataDay);
            formatter.dateFormat = [[NSString alloc] initWithFormat:@"%@",Localized(@"Refresh.today")];
        ...
        // 3.显示日期
        self.lastUpdatedTimeLabel.text = [NSString stringWithFormat:@"%@ %@", [[NSString alloc] initWithFormat:@"%@",Localized(@"Last.update")],time];
        
    } else {
        self.lastUpdatedTimeLabel.text = [[NSString alloc] initWithFormat:@"%@",Localized(@"LastUpdateNoRecords")];
    }
}
